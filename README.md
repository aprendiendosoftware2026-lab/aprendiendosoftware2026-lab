<div align="center">

        < / >       { }       [ ]

    PHP        =>        SQL        JS

          &&       ||       !=

             A R

        // Código • Diseño
👩‍💻 ¡Hola! Soy Amalia Restrepo </div>

### Estudiante de Programación de Software | 🎨 Diseñadora Gráfica

¡Bienvenido(a) a mi perfil de GitHub!

Actualmente estoy cursando una Técnica en Programación de Software, donde he adquirido conocimientos en desarrollo web, bases de datos, lógica de programación y control de versiones. A través de proyectos académicos y personales continúo fortaleciendo mis habilidades y explorando nuevas tecnologías.

Además, mi formación en Diseño Gráfico me permite aportar una perspectiva creativa al desarrollo de interfaces y experiencias de usuario.

---

## 🚀 Tecnologías y herramientas

<p align="left">
  <img src="https://skillicons.dev/icons?i=html,css,js,php,mysql,git,github,vscode" />
</p>

---

## 🌱 Actualmente aprendiendo

### ⚙️ Desarrollo Backend

<p align="left">
  <img src="https://skillicons.dev/icons?i=php" height="55" alt="PHP"/>
</p>

### 🗄️ Bases de Datos

<p align="left">
  <img src="https://skillicons.dev/icons?i=mysql" height="55" alt="MySQL"/>
</p>

### 🔧 Control de versiones

<p align="left">
  <img src="https://skillicons.dev/icons?i=git,github" height="55" alt="Git y GitHub"/>
</p>

### 🌐 Desarrollo de aplicaciones web

<p align="left">
  <img src="https://skillicons.dev/icons?i=html,css,js" height="55" alt="HTML CSS JavaScript"/>
</p>

### 🧹 Buenas prácticas de programación

💡 Código limpio · Organización · Estructura · Reutilización

### 🗂️ Diseño y modelado de bases de datos

📊 Modelado · Relaciones · Tablas · Consultas SQL


---

## 📂 Mis proyectos

En este perfil encontrarás proyectos desarrollados durante mi proceso de aprendizaje, donde aplico los conocimientos adquiridos y continúo fortaleciendo mis habilidades como desarrolladora.

Algunos temas que encontrarás son:

* 🌐 Desarrollo web.
* 🗄️ Bases de datos relacionales.
* 👤 Sistemas de autenticación de usuarios.
* ⚙️ CRUD con PHP y MySQL.
* 🚲 Proyectos académicos de gestión de bicicletas.
* 📚 Ejercicios y prácticas de programación.

---

## 🎯 Mi objetivo

Mi meta es crecer como desarrolladora de software, adquirir experiencia en proyectos reales y seguir aprendiendo nuevas tecnologías que me permitan crear soluciones útiles y de calidad.

Creo que cada proyecto representa una oportunidad para aprender algo nuevo y mejorar tanto a nivel técnico como personal.

---

### ✨ Un poco sobre mí

* 💜 Me encanta aprender cosas nuevas cada día.
* 🎨 Disfruto combinar el diseño gráfico con el desarrollo de software.
* 💡 Me motivan los retos y la resolución de problemas.
* 🚀 Actualmente construyendo mi portafolio de proyectos.

> **"Cada línea de código es una oportunidad para aprender, mejorar y crear algo útil."**
